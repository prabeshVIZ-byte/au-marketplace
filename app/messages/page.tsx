...
export default function ThreadPage() {
  ...
  const [bottomNavH, setBottomNavH] = useState(92);

  useEffect(() => {
    const update = () => {
      const nav = document.querySelector("nav");
      const navH = nav ? Math.round((nav as HTMLElement).getBoundingClientRect().height) : 92;
      setBottomNavH(Math.max(80, Math.min(130, navH || 92)));
    };
    update();
  ...
  const COMPOSER_H = 78;
  const FLOATING_BANNER_H = replyTo || editingId ? 64 : 0;
  const reservedBottom = bottomNavH + COMPOSER_H + FLOATING_BANNER_H + 30;
    ...
    >
      ...
      {showOptionsSheet && (
        <div
          ...
          style={{ paddingBottom: bottomNavH + COMPOSER_H + 14 }}
        >
      ...
      <style jsx>{`
        ...
        .jumpBtn {
          position: fixed;
          right: 14px;
          bottom: calc(${bottomNavH}px + ${COMPOSER_H + 16}px);
          height: 42px;
          padding: 0 14px;
          border-radius: 999px;
          border: 1px solid rgba(15, 23, 42, 0.08);
